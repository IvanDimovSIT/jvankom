...
pub fn if_compare_equals_instruction(context: JvmContext) -> JvmResult<()> {
    generic_int_if_compare_instruction(context, |a, b| a == b)
}
...
pub fn if_reference_equals_instruction(context: JvmContext) -> JvmResult<()> {
    generic_reference_if_compare_instruction(context, |a, b| a == b)
}

pub fn if_reference_not_equals_instruction(context: JvmContext) -> JvmResult<()> {
    generic_reference_if_compare_instruction(context, |a, b| a != b)
}
...
#[inline]
fn generic_int_if_compare_instruction<F>(context: JvmContext, logic_fn: F) -> JvmResult<()>
where
...
#[inline]
fn generic_reference_if_compare_instruction<F>(context: JvmContext, logic_fn: F) -> JvmResult<()>
where
    F: FnOnce(Option<NonZeroUsize>, Option<NonZeroUsize>) -> bool,
{
    let frame = context.current_thread.top_frame();
    let value2 = pop_reference(frame)?;
    let value1 = pop_reference(frame)?;
    if !logic_fn(value1, value2) {
        // skip branch location
        frame.program_counter += 2;
        return Ok(());
